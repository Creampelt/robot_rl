...
def _rebuild_fbcpr(train_cfg: dict, ckpt: dict, all_models: bool) -> dict[str, nn.Module]:
    ...
    def build(name: str) -> nn.Module:
        ...
        if dist_cfg is not None:
            ...
            # construct_algorithm force-overrides these bounds with clip_actions at train time; raw cfg
            # dumps keep the class default (+/-1) -- mirroring setdefault here silently squashes actions
            if dist_cfg["class_name"] == "TruncatedGaussianDistribution":
                dist_cfg["low"] = -cfg["clip_actions"]
                dist_cfg["high"] = cfg["clip_actions"]
